...
const char *versionString =
     "reduce: version 4.10 2/11/2023, Copyright 1997-2016, J. Michael Word; 2020-2023 Richardson Lab at Duke University";

const char *shortVersion    = "reduce.4.10.230211";
    ...
    for (std::list< std::shared_ptr<PDBrec> >::const_iterator ptr = l.begin(); ptr != l.end(); ++ptr) {
        ...
        if ((*ptr)->valid() && flag) {
          ...
          if (dumpFile && ((t == PDB::ATOM) || (t == PDB::HETATM))) {
            ...
            // Chain, Residue name, Residue number, atom name, alt, x,y,z, vdw_radius,
            // is acceptor, is donor, is metallic
            fprintf(dumpFile, "%s %s %3d %-4s %c %7.3f %7.3f %7.3f %5.2f %s %s %s\n",
              rec->chain(), rec->resname(), rec->resno(), rec->atomname(), alt,
