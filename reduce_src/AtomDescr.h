...
{
public:
	AtomDescr() : _atomPos(0,0,0), _atomResNo(0), _atRadius(0), _originalAtom( 0 ) {};
	AtomDescr(Point3d atomPos, int resNum, float atRadius) : _atomPos(atomPos), _atomResNo(resNum), _atRadius(atRadius), _originalAtom( 0 ) {};
	~AtomDescr() {};
	AtomDescr(const AtomDescr& rhs) {_atomPos = rhs._atomPos; _atomResNo = rhs._atomResNo; _atRadius = rhs._atRadius; _originalAtom = rhs._originalAtom;};
	
	void setAtomPos(Point3d sn) {_atomPos = sn; return;}
	void setAtomResNum(int rn) {_atomResNo = rn; return;};
	void setAtRadius(float atRadius) {_atRadius = atRadius;return;};
	...
	bool operator==(const AtomDescr& rhs) const {return ((_atomResNo == rhs._atomResNo) && (_atRadius == rhs._atRadius) && (distanceSquared(_atomPos, rhs._atomPos) < .000001 ) );};
	...
	bool intersects(const AtomDescr& a) const { return (distanceSquared(a._atomPos, _atomPos) < (a._atRadius + _atRadius) * (a._atRadius + _atRadius));}	//ANDREW
