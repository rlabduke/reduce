...
void AtomPositions::put(PDBrec* r) {
  ...
  neighbor_list.insert(temp, LocBlk(r->loc()));
  r->set_index(temp->index());
}

std::list< PointWith<PDBrec*> > AtomPositions::neighbors(const Point3d& p, Coord mindist, Coord maxdist) {
  return neighbor_list.get_neighbors(p, mindist, maxdist);
}
...
#if USE_SYM
  neighbor_list.reposition(rec.index(),rec.loc(),neighbor_list._atom_list[rec.index()]);

#else
   LocBlk pb(prev);
   LocBlk nb(rec.loc());
   if (! (pb == nb)) { // rec has moved to a new xyz block
	   std::multimap<LocBlk, PDBrec*>::iterator it = neighbor_list.find(pb);
	   std::multimap<LocBlk, PDBrec*>::iterator it2;
	   PDBrec* a = NULL;
	   while (it != neighbor_list.end() && it->first == pb) {
		   a = it->second;
		   it2 = it;
		   ++it;
		   if (a->valid() && *a == rec) {
			   delete it2->second;
			   neighbor_list.erase(it2);
		   }
	   }
	   PDBrec* r = new PDBrec(rec);
	   neighbor_list.insert(r, nb);
   }
#endif
}
...
//FindCliques uses some cctbx, so there's a lot of preprocessor statements in here
// ---------------------------------------------------------------
#if USE_SYM
...
#if USE_SYM
	NeighborList<BumperPoint*> bumpbins(cell,space_grp,distance_cutoff);
#else
        NeighborList<BumperPoint*> bumpbins;
#endif
...
// exclude bumper points which interact with exclude-list members
	...
	for (std::list<BumperPoint*>::const_iterator xp_iter = _excludePoints.begin(); xp_iter != _excludePoints.end(); ++xp_iter) {
		xp = *xp_iter;
		std::list< PointWith<BumperPoint*> > nearxp_list = bumpbins.get_neighbors(xp->loc(), (Coord)0.001,
			(Coord)(xp->vdwRad() + prRad + maxVDWrad));
		std::list< PointWith<BumperPoint*> >::iterator nearxp = nearxp_list.begin();
		while (nearxp != nearxp_list.end()) {
			if (nearxp->getAtom()->valid()
				&& ((distance2(xp->loc(), nearxp->getPoint())
				- xp->vdwRad() - nearxp->getAtom()->vdwRad()) < prRad)) {

				nearxp->getAtom()->invalidate(); // turn off if bumps exclude pt
			}
			++nearxp;
		}
	}
...
//	add connections between memos to DisjointSet (& remember pairwise links)
...
#if USE_SYM
		for (int i=0; i<bumpbins._atom_list.size(); i++){
			bp = bumpbins._atom_list[i];
#else
	...
	while (bbit != bumpbins.end()) {
		...
		for (; bbit != bumpbins.end() && bbit->first == *key; ++bbit) {
			bp = bbit->second;
#endif
			std::list< PointWith<BumperPoint*> > nearbp_list = bumpbins.get_neighbors(bp->loc(), (Coord)0.001,
				(Coord)(bp->vdwRad() + prRad + maxVDWrad));
			std::list< PointWith<BumperPoint*> >::iterator nearbp = nearbp_list.begin();
			while (nearbp != nearbp_list.end()) {
				const int ares = bp->rnum(), bres = nearbp->getAtom()->rnum();
				if ((ares != bres) && bp->valid() && nearbp->getAtom()->valid()
					&& (!sameres(memo[ares]->exampleAtom(), memo[bres]->exampleAtom())) //Vishal: Check if same res but diff sym
					&& ((distance2(bp->loc(), nearbp->getPoint())
					- bp->vdwRad() - nearbp->getAtom()->vdwRad()) < prRad)) {
...
void AtomPositions::CollectBumping(const AtomDescr& ad, std::list<PDBrec*>& bumping)
{
  ...
  std::list< PointWith<PDBrec*> > nearby = neighbor_list.get_neighbors(ad.getAtomPos(), (Coord)0.001,
      		(Coord)(ad.getAtRadius() + _probeRadius + _maxVDWFound ));

	PDBrec* ni = NULL;
	for(std::list< PointWith<PDBrec*> >::iterator it = nearby.begin(); it != nearby.end(); ++it) {
		ni = it->getAtom();
...
void AtomPositions::generateWaterPhantomHs(std::list<PDBrec*>& waters) {
	...
	for(std::list<PDBrec*>::const_iterator it = waters.begin(); it != waters.end(); ++it) {
		...
		//master version in comments in case this doesn't compile / is wrong
		...
		std::list< PointWith<PDBrec*> > nearby_list = neighbors(a->loc(), 0.001, 4.0);
		PDBrec* rec = NULL;
		for(std::list< PointWith<PDBrec*> >::const_iterator nearby = nearby_list.begin(); nearby != nearby_list.end(); ++nearby) {
			rec = nearby->getAtom();
		
			if (rec->hasProp(ACCEPTOR_ATOM)
				|| FlipMemo::isHBDonorOrAcceptorFlipped(*rec, _useXplorNames, _useOldNames, _bbModel)) {
				
				double HBoverlap = distance2(a->loc(), nearby->getPoint())
				...
				if (HBoverlap < -0.01
						...
						for (i=0; i < nAcc; i++) {
							if (resDescr == nearbyA[i]._nam) {
								if (HBoverlap < nearbyA[i]._gap) {
									...
									nearbyA[i]._loc = nearby->getPoint();
					...
					if (!foundMatchingAromAtom && nAcc < MaxAccDir) {
						...
						nearbyA[nAcc]._loc = nearby->getPoint();
...
#define BEST_HBOND_OVERLAP 0.6
			...
			neighbor_list.insert(pHatom, LocBlk(pHatom->loc()));
#ifdef DUMPPHANH
...
double AtomPositions::atomScore(const PDBrec& a, const Point3d& p,
	...
	std::list< PointWith<PDBrec*> > nearby = this->neighbors( p, 0.001, nearbyRadius);

	std::list< PointWith<PDBrec*> > bumping_list; // first we collect atoms actually interacting
	PDBrec* rec = NULL;
	for (std::list< PointWith<PDBrec*> >::const_iterator ni = nearby.begin(); ni != nearby.end(); ++ni) {
		rec = ni->getAtom();
		if (((a==*rec) && (distanceSquared(a.loc(),ni->getPoint())>0.00001))){
			std::cerr<<"Close copy "<<a.index()<<std::endl;
		}
		if (rec->valid() && (! rec->hasProp(IGNORE))
			&& (abs(rec->occupancy()) > _occupancyCuttoff)
			&& (! ((a == *rec) && (distanceSquared(a.loc(),ni->getPoint())<0.00001)))	// Check location here?
			&& (interactingConfs(a, *rec, _onlyA))
			&& ( vdwGap(a, p, *rec, ni->getPoint()) < pRad)
                        && (! foundInList(PointWith<PDBrec*>(rec, ni->getPoint()), exclude))) {	// What about symmetric copies?
			bumping_list.push_back(*ni);
	}
	}


	std::vector< PointWith<PDBrec*> > bumping;
	...
	for (int i=0; i < ndots; i++) // then we inspect each dots interactions
	{
		...
		for (int ii = 0; ii < bumping.size(); ++ii) {
			b = bumping[ ii ].getAtom();
			const Point3d locb = bumping[ ii ].getPoint();
