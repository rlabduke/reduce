...
void processPDBfile(std::istream& ifs, char *pdbFile, std::ostream& ofs) {
   ...
   if (RemoveHydrogens) {
	...
	NeighborList nget(6.0);

	scitbx::af::double6 cell(cryst1.a,cryst1.b,cryst1.c,cryst1.alpha,cryst1.beta,cryst1.gamma);
	int num_atoms=nget.init(xyz.get_xyzBlocks(), (scitbx::af::double6)cell, cryst1.spaceGrp);

	for (int i=0; i<num_atoms; i++){
		printf("%d :",i);
		nlist=nget.get_neighbors(i,0.0);
		for (std::list<int>::iterator itr=nlist.begin(); itr!=nlist.end(); itr++)
			printf("%d ",*itr);
		printf("\n");
	}
    }
