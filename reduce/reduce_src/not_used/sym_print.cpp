...
//      NonConstListIter<PDBrec> infoPtr(records); // info on changes can be
	...
	xyz.init_neighborList(cell, cryst1.spaceGrp, 2*ElementInfo::StdElemTbl().maxCovalentRadius());
      scanAndGroupRecords(records, xyz, infoPtr);
	xyz.cubiclize();

	char *coordFile;
	int len=strlen(pdbFile);
	coordFile=new char[len+8];
	strcpy(coordFile,pdbFile);
	strcpy(coordFile+len,".coords");
	cerr<<coordFile<<endl;
	std::ofstream fptr(coordFile);
	xyz.print_all(fptr,ofs);
	fptr.close();
	delete coordFile;
...
bool okToPlaceHydHere(const PDBrec& theHatom, const atomPlacementPlan& pp,
	...
	if (pp.hasFeature(ROTATEFLAG) || pp.hasFeature(NH3FLAG)) {
		...
		if (heavyAtom.elem().atno() != 6) { // OH, SH and NH3, but not CH3
			...
			std::list< std::pair<PDBrec*, Point3d> > sym_nearr_list = xyz.get_neighbors( heavyAtom.loc(),  
				halfbondlen + 0.1,
				maxbondlen  + 0.25);
//			std::cerr<<"Neighbor lookup for "<<heavyAtom.index()<<" found "<<sym_nearr_list.size()<<" neighbors "<<endl;
/*			std::list<PDBrec*> nearr_list = xyz.neighbors( heavyAtom.loc(),
					...
					if ((actual >= (expected - 0.55))
						...
						if (++countOfBonds >= 2) {
							...
							if (pp.hasFeature(NH3FLAG)) {
								...
								if (skipthisH) {
									recordSkipInfo(TRUE, fixNotes, theHatom, heavyAtom, old_c2batoms, "(NH2R)");
	...
	if (pp.hasFeature(BONDBUMPFLAG)
		...
		std::list< std::pair<PDBrec*, Point3d> > sym_nearr_list = xyz.get_neighbors( nqoxygen.loc(),  
			halfbondlen + 0.1,
			maxbondlen  + 0.25);
//		std::cerr<<"Neighbor lookup for "<<nqoxygen.index()<<" found "<<sym_nearr_list.size()<<" neighbors "<<endl;
...
/*		std::list<PDBrec*> nearr_list = xyz.neighbors( heavyAtom.loc(),
	...
	if (pp.hasFeature(IFNOPO4)) {
		...
		std::list< std::pair<PDBrec*, Point3d> > sym_nearr_list = xyz.get_neighbors( theoxygen.loc(),  
			pobondlen - 0.25, pobondlen + 0.25);
//		std::cerr<<"Neighbor lookup for "<<theoxygen.index()<<" found "<<sym_nearr_list.size()<<" neighbors "<<endl;
