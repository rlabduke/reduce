...
void RotDonor::finalize(int nBondCutoff, bool useXplorNames, bool useOldNames, bool bbModel, 
	...
	if (isComplete()) {
		...
		std::list< PointWith<PDBrec*> > nearby_list = xyz.neighbors(_heavyAtom.loc(), 0.001, 4.0);
		PDBrec* rec = NULL;
		for(std::list< PointWith<PDBrec*> >::const_iterator nearby = nearby_list.begin(); nearby != nearby_list.end(); ++nearby) {
			...
			if (rec->hasProp(ACCEPTOR_ATOM)
				...
				double HBoverlap = distance2(_heavyAtom.loc(), nearby->getPoint())
				...
				if (HBoverlap < OVERLAPCUTOFF_1
					...
					const double ang = origAngle +
						dihedral(nearby->getPoint(), _p1, _p2, (*(_rot.begin()))->loc());
