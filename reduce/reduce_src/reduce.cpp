...
#include "PointWith.h"
bool NoSym        = FALSE;
bool IS_A_CRYSTAL=false; // Vishal: If true, use CCTBX
struct Cryst1 {
        double    a, b, c;
...
void getBBox(std::list<PDBrec*>& rlst, Coord bbox[]);//, Coord box_center[]);
...
void recordSkipInfo(bool skipH, std::vector<std::string>& fixNotes,
   ...
   std::list<PointWith<PDBrec*> >& nearr, const char * msg);
...
//      NonConstListIter<PDBrec> infoPtr(records); // info on changes can be
      ...
      double ucell_params[3];
      xyz.get_unit_cell_params(ucell_params);
      cell[0]=ucell_params[0];
      cell[1]=ucell_params[1];
      cell[2]=ucell_params[2];
      fprintf(stderr,"After: %s %f %f %f\n", cryst1.spaceGrp, cell[0],cell[1],cell[2]);
...
#if USE_SYM
         CliqueList clst = xyz.findCliques(cell, cryst1.spaceGrp, 2*ElementInfo::StdElemTbl().maxCovalentRadius());
#else
	...
	for (std::list<PDBrec*>::iterator it = rlst.begin(); it != rlst.end(); ++it) {
		...
		if (r->type() == PDB::ATOM) {
			if (! foundStart) {
				...
				if (r->x() < box_min[0])
					box_min[0]=r->x();
				else if (r->x() > box_max[0])
					box_max[0]=r->x();

				if (r->y() < box_min[1])
					box_min[1]=r->y();
				else if (r->y() > box_max[1])
					box_max[1]=r->y();

				if (r->z() < box_min[2])
					box_min[2]=r->z();
				else if (r->z() > box_max[2])
					box_max[2]=r->z();
			}
		}
	}
	bbox[0]=box_max[0]-box_min[0];
	bbox[1]=box_max[1]-box_min[1];
	bbox[2]=box_max[2]-box_min[2];
//	box_center[0]=(box_max[0]+box_min[0])/2;
//	box_center[1]=(box_max[1]+box_min[1])/2;
//	box_center[2]=(box_max[2]+box_min[2])/2;
}
...
bool okToPlaceHydHere(const PDBrec& theHatom, const atomPlacementPlan& pp,
	...
	std::list< PointWith<PDBrec*> > emptySet;
	...
	if (pp.hasFeature(ROTATEFLAG) || pp.hasFeature(NH3FLAG)) {
		...
		if (heavyAtom.elem().atno() != 6) { // OH, SH and NH3, but not CH3
			...
			std::list< PointWith<PDBrec*> > sym_nearr_list = xyz.neighbors( heavyAtom.loc(),
				halfbondlen + 0.1,
				maxbondlen  + 0.25);
			std::list< PointWith<PDBrec*> > c2batoms;
			int countOfBonds = 0;
			PDBrec* rec = NULL;
			for (std::list< PointWith<PDBrec*> >::const_iterator nearr = sym_nearr_list.begin(); nearr != sym_nearr_list.end(); ++nearr) {
				rec = nearr->getAtom();
					...
					if ((actual >= (expected - 0.55))
						...
						if (++countOfBonds >= 2) {
							...
							if (pp.hasFeature(NH3FLAG)) {
								...
								PDBrec* nbs = NULL;
								for(std::list< PointWith<PDBrec*> >::const_iterator it = c2batoms.begin(); it != c2batoms.end(); ++it) {
									nbs = it->getAtom();
	...
	if (pp.hasFeature(BONDBUMPFLAG)
		...
		std::list< PointWith<PDBrec*> > sym_nearr_list = xyz.neighbors( nqoxygen.loc(),
		...
		for (std::list< PointWith<PDBrec*> >::const_iterator nearr = sym_nearr_list.begin(); nearr != sym_nearr_list.end(); ++nearr) {
			...
			if (interactingConfs(nqoxygen, *rec, DoOnlyAltA)
				...
				const double actual = distance2(nqoxygen.loc(), nearr->getPoint());
				...
				if ((actual >= (expected - 0.65))
					...
					emptySet.push_front(PointWith<PDBrec*>(rec,rec->loc()));
	...
	if (pp.hasFeature(BONDBUMPFLAG) && (!doNotAdjustSC)) {
		...
		for (std::list< PointWith<PDBrec*> >::const_iterator nearr = sym_nearr_list.begin(); nearr != sym_nearr_list.end(); ++nearr) {
			rec = nearr->getAtom();
			bool is_sym=(distanceSquared(rec->loc(), nearr->getPoint()) > 0.00001);  //Vishal: use TOL
			if (interactingConfs(heavyAtom, *rec, DoOnlyAltA)
				&& (! (sameres(theHatom, *rec, is_sym)
				&& StdResH::ResXtraInfo().match(theHatom.resname(), "AminoAcid") ))
				&& (! rec->elem().isHydrogen())
				&& (! rec->isWater()) ) {
				const double actual = distance2(heavyAtom.loc(), nearr->getPoint());
	...
	if (pp.hasFeature(IFNOPO4)) {
		...
		std::list< PointWith<PDBrec*> > nearr_list = xyz.neighbors( theoxygen.loc(),
			pobondlen - 0.25, pobondlen + 0.25);
		PDBrec* rec = NULL;
		for (std::list< PointWith<PDBrec*> >::const_iterator nearr = nearr_list.begin(); nearr != nearr_list.end(); ++nearr) {
			rec = nearr->getAtom();
...
void recordSkipInfo(bool skipH, std::vector<std::string>& fixNotes,
	...
	PDBrec* rec = NULL;
	for(std::list< PointWith<PDBrec*> >::const_iterator it = nearr.begin(); it != nearr.end(); ++it) {
		rec = it->getAtom();
		bool is_sym=(distanceSquared(rec->loc(), it->getPoint()) > 0.00001);  //Vishal: use TOL
