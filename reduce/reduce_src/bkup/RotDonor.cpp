...
void RotDonor::finalize(int nBondCutoff, bool useXplorNames, bool useOldNames, bool bbModel, 
	...
	if (isComplete()) {
		...
		std::list< std::pair<PDBrec*, Point3d> > nearby_list = xyz.get_neighbors(_heavyAtom.loc(), 0.001, 4.0);
		PDBrec* rec = NULL;
...
int RotDonor::makebumpers(std::multimap<LocBlk, BumperPoint*>& bblks, NeighborList<BumperPoint*>& sym_bblks, 
