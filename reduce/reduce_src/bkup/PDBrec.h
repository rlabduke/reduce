...
public:
   ...
   PDBrec(const PDB& r) : _rep(new PDBrecRep()), _i(new int(1)) { 
								_rep->update(r);
								 _index=-1;
								}
  ~PDBrec() {
   ...
   PDBrec(const PDBrec& a): _rep(a._rep), _i(a._i) {
						++*_i;
						_index = a._index;
						}
   ...
   void set_chain_id(int num)    const { _rep->_r.atom.residue.chainId[1]=num; }
...
private:
   ...
   PDBrecRep* _rep;
   int* _i;
   int _index; /// vishal: for indexing
};
