...
void AtomPositions::reposition(const Point3d& prev, const PDBrec& rec) {
   LocBlk pb(prev);
   LocBlk nb(rec.loc());
   PDBrec* tmp = new PDBrec(rec);
 
  int tmp_j=_sym_neighbor_list.find(tmp,prev);
   ...
   if (! (pb == nb)) { // rec has moved to a new xyz block
	   ...
	   while (it != _xyzBlocks.end() && it->first == pb) {
		   ...
		   ++it;
		   if (a->valid() && *a == rec) {
   			   _sym_neighbor_list.reposition(tmp_j,rec.loc(),r);
			   delete it2->second;	// *********When removing _xyzBlocks replace this by deleting the pointer through _sym_ngbr_list
	...
	_sym_neighbor_list.reposition(tmp_j,rec.loc(),_sym_neighbor_list._atom_list[tmp_j]);
...
//	Vector< MoverPtr >   memo(mdsz);
	...
	while (it != _motionDesc.end()) {
		...
		if (mx != NULL && mx->valid() && mx->isComplete()
			...
			an += mx->makebumpers(bumpbins, sym_bumpbins, rn, maxVDWrad);
			++rn;
		}
		++it;
	}
...
//		for (; bbit != bumpbins.end() && bbit->first == *key; ++bbit) { // Commented by Visha;
///////////////////////////////////////////////////////////////////////////////////////
...
void AtomPositions::generateWaterPhantomHs(std::list<PDBrec*>& waters) {
	...
	for(std::list<PDBrec*>::const_iterator it = waters.begin(); it != waters.end(); ++it) {
		a = *it;
		int i = 0, nAcc = 0;
		std::list< std::pair<PDBrec*,Point3d> > nearby_list = get_neighbors(a->loc(), 0.001, 4.0);
		PDBrec* rec = NULL;
...
double AtomPositions::atomScore(const PDBrec& a, const Point3d& p,
	...
	std::list< std::pair<PDBrec*,Point3d> > nearby = this->get_neighbors( p, 0.001, nearbyRadius);
